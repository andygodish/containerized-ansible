...
# Create README.md
# NOTE: This heredoc is single-quoted so bash does *not* execute backticks/$(...) while bootstrapping.
README_PATH="$PROJECT_DIR/README.md"
cat > "$README_PATH" << 'EOF'
# __PROJECT_NAME__

Ansible playbook project for __PROJECT_NAME__.
...
# substitute placeholders (portable sed -i across GNU/BSD)
if sed --version >/dev/null 2>&1; then
  ...
  # BSD/macOS sed requires an extension argument (empty string for none)
  sed -i '' \
    -e "s/__PROJECT_NAME__/${PROJECT_NAME}/g" \
    -e "s#__PROJECT_DIR__#${PROJECT_DIR}#g" \
    "$README_PATH"
